...
public class CmacTest {
    ...
    public void testCmac() throws KrbException {
        byte[] key = Util.hex2bytes2(keyBytes);
        byte[] input = Util.hex2bytes2(inputBytes);
        EncryptProvider encProvider = new Camellia128Provider();
        byte[] result;

        // test 1
        result = Cmac.cmac(encProvider, key, input, 0, 0);
        Assert.assertArrayEquals("Test 1", Util.hex2bytes2(cmac1), result);

        // test 2
        result = Cmac.cmac(encProvider, key, input, 0, 16);
        Assert.assertArrayEquals("Test 2", Util.hex2bytes2(cmac2), result);

        // test 3
        result = Cmac.cmac(encProvider, key, input, 0, 40);
        Assert.assertArrayEquals("Test 3", Util.hex2bytes2(cmac3), result);

        // test 4
        result = Cmac.cmac(encProvider, key, input, 0, 64);
        Assert.assertArrayEquals("Test 4", Util.hex2bytes2(cmac4), result);
    }
}
