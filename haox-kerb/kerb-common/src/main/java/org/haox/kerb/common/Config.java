...
public class Config {
     ...
     *
     * @exception sun.security.krb5.KrbException if error occurs when constructing a Config
     ...
     * @throws java.io.IOException if there is an I/O or format error
     */
    ...
    public int getType(String input) {
        ...
        if (input.startsWith("d") || (input.startsWith("D"))) {
            if (input.equalsIgnoreCase("des-cbc-crc")) {
                ...
                result = sun.security.krb5.Checksum.CKSUMTYPE_DES_MAC;
            } else if (input.equalsIgnoreCase("des-mac-k")) {
                result = sun.security.krb5.Checksum.CKSUMTYPE_DES_MAC_K;
        ...
        // RC4-HMAC
            ...
            result = sun.security.krb5.Checksum.CKSUMTYPE_CRC32;
        } else if (input.startsWith("r") || (input.startsWith("R"))) {
            if (input.equalsIgnoreCase("rsa-md5")) {
                result = sun.security.krb5.Checksum.CKSUMTYPE_RSA_MD5;
            } else if (input.equalsIgnoreCase("rsa-md5-des")) {
                result = sun.security.krb5.Checksum.CKSUMTYPE_RSA_MD5_DES;
            }
        } else if (input.equalsIgnoreCase("hmac-sha1-des3-kd")) {
            result = sun.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_DES3_KD;
        } else if (input.equalsIgnoreCase("hmac-sha1-96-aes128")) {
            result = sun.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_96_AES128;
        } else if (input.equalsIgnoreCase("hmac-sha1-96-aes256")) {
            result = sun.security.krb5.Checksum.CKSUMTYPE_HMAC_SHA1_96_AES256;
    ...
    /**
     * Gets default realm.
     * @throws sun.security.krb5.KrbException where no realm can be located
     ...
     * @throws sun.security.krb5.KrbException if there's no way to find KDC for the realm
    ...
    private String getRealmFromDNS() throws KrbException {
        ...
        try {
            ...
            KrbException ke = new KrbException(KrbErrorCode.KRB_ERR_GENERIC, e);
            ke.initCause(e);
            throw (ke);
        }
        // get the domain realm mapping from the configuration
        String mapRealm = mapHostToRealm(hostName);
        if (mapRealm == null) {
        ...
        if (realm == null) {
            throw new KrbException(KrbErrorCode.KRB_ERR_GENERIC,
                                "Unable to locate Kerberos realm");
        }
        return realm;
    }

    public static String mapHostToRealm(String name) {
        String result = null;
        try {
            String subname = null;
